...
var PageTurn = (function () {
    ...
    /**
     * 
     * @param {String} domId 容器 id:"#id"/class:".classname"
    ...
    PageTurn.prototype.getUniqueId = function () {
        var _id = "pageturn-box" + Math.ceil(Math.random() * 500);
    ...
    PageTurn.prototype._init = function () {
        ...
        var str = '<div class="pageturn-box page-info">' + (options.disableTips ? '' : '<div class="record-text">共<span id="totalCount"></span>条记录 第<span id="curPageIndex">1</span>/<span id="totalPage">1</span>页</div>') + '<div class="page-turn"><div class="pre-page fl user-noselect-text">'+ (options.prePageIcon ? '<img src="' + options.prePageIcon + '" />' : '&lt;') +'</div><ul id="ul-tab" class="fl"></ul><div class="next-page fl user-noselect-text">' + (options.nextPageIcon ? '<img src="' + options.nextPageIcon + '" />' : '&gt;') + '</div>'+ (options.disableOptions ? '' : '<select class="fl" name="" id="everyPageNum"></select>') + (options.disableJump ? '' : '<div class="fl jumpto">跳至<input type="text" id="jumto-input" value="1">页</div>') + '</div></div>';
    ...
    PageTurn.prototype.refreshList = function (notrerenderLi) {
        ...
        if (totalPage <= 1) {
            ...
            this.prePageDom.style.display = "flex";
            this.nextPageDom.style.display = "flex";
        }
